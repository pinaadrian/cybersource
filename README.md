...
## Installation
You can install the client either via [Composer](https://getcomposer.org/). Before installing, make sure to configure the merchant ID, transaction key, and the appropriate WSDL file URL in ````config/cybersource.php````. By default, the WSDL file for the client is for API version 1.120. Available WSDL file URLs can be browsed at the following locations:
...
composer require adrianxplay/cybersource
```
Then, to use the client, you'll need to include the following class to your app:

```php
use Adrianxplay\Cybersource\CybsClient;
